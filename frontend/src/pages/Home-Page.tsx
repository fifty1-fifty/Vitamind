...
const HomePage = () =>
{
  ...
  /*const handleDateOffset = (op : string) =>
  {
    let getDateOffset = dateOffset;
    console.log(getDateOffset);
    //console.log('handleDateOffset ' + dateOffset);
    if(op == '-')
    {
      getDateOffset = getDateOffset - 1;
      setDateOffset(getDateOffset);
      console.log('this print indicates you wanted to minus ' + dateOffset);
      loadDailyStats();
    }
    else if(op == '+' && dateOffset < 0)
    {
      getDateOffset = getDateOffset + 1;
      setDateOffset(getDateOffset);
      console.log('this print you wanted to PLUS' + dateOffset);
      loadDailyStats();
    }
        ...
        <div className="row justify-content-center align-items-start">
            ...
            <div className='row align-items-end justify-content-center'>
               <div className='col align-self-end' id='nuts'>
                    {toggleScanner === 'first' && <DailyBar text={'Calories'} progressPercent={currentDayProductPercent.calories} progressValue={currentDayProductValue.calories}/>}  
                    {toggleScanner === 'first' && <DailyBar text={'Protein'} progressPercent={currentDayProductPercent.protein} progressValue={currentDayProductValue.protein}/>} 
                    {toggleScanner === 'first' && <DailyBar text={'Carbohydrates'} progressPercent={currentDayProductPercent.carbohydrates} progressValue={currentDayProductValue.carbohydrates}/>}  
                    {toggleScanner === 'first' && <DailyBar text={'Cholesterol'} progressPercent={currentDayProductPercent.cholesterol} progressValue={currentDayProductValue.cholesterol}/>}
                    {toggleScanner === 'first' && <DailyBar text={'Total Fats'} progressPercent={currentDayProductPercent.totalFats} progressValue={currentDayProductValue.totalFats}/>}
                    {toggleScanner === 'first' && <DailyBar text={'Saturated Fats'} progressPercent={currentDayProductPercent.saturatedFats} progressValue={currentDayProductValue.saturatedFats}/>}
                    {toggleScanner === 'first' && <DailyBar text={'Sodium'} progressPercent={currentDayProductPercent.sodium} progressValue={currentDayProductValue.sodium}/>}
                    {toggleScanner === 'first' && <DailyBar text={'Sugar'} progressPercent={currentDayProductPercent.sugar} progressValue={currentDayProductValue.sugar}/>}
                    {toggleScanner === 'first' && <DailyBar text={'Fiber'}progressPercent={currentDayProductPercent.fiber} progressValue={currentDayProductValue.fiber}/>}
                    {toggleScanner === 'first' && <DailyBar text={'Potassium'}progressPercent={currentDayProductPercent.potassium} progressValue={currentDayProductValue.potassium}/>}
                    {toggleScanner === 'first' && <DailyBar text={'Calcium'}progressPercent={currentDayProductPercent.calcium} progressValue={currentDayProductValue.calcium}/>}
                    {toggleScanner === 'first' && <DailyBar text={'Magnesium'}progressPercent={currentDayProductPercent.magnesium} progressValue={currentDayProductValue.magnesium}/>}
